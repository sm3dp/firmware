...
volatile float max_feedrate[4] = DEFAULT_MAX_FEEDRATE;					//������������ٶ�?
volatile volatile float axis_steps_per_unit[4] = DEFAULT_AXIS_STEPS_PER_UNIT;	//�ƶ�1mm��step��
volatile unsigned long max_acceleration_units_per_sq_second[4] = DEFAULT_MAX_ACCELERATION;	//�����ٶ�
volatile unsigned long axis_steps_per_sqr_second[NUM_AXIS];

volatile unsigned long minsegmenttime = DEFAULT_MINSEGMENTTIME;
volatile float minimumfeedrate = DEFAULT_MINIMUMFEEDRATE;;
volatile float acceleration = DEFAULT_ACCELERATION;;       // Normal acceleration mm/s^2  THIS IS THE DEFAULT ACCELERATION for all moves. M204 SXXXX
volatile float retract_acceleration = DEFAULT_RETRACT_ACCELERATION;; //  mm/s^2   filament pull-pack and push-forward  while standing still in the other axis M204 TXXXX
volatile float max_xy_jerk = DEFAULT_XYJERK; //speed than can be stopped at once, if i understand correctly.
volatile float max_z_jerk = DEFAULT_ZJERK;
volatile float max_e_jerk = DEFAULT_EJERK;
volatile float mintravelfeedrate = DEFAULT_MINTRAVELFEEDRATE;;



// The current position of the tool in absolute steps
volatile long position[4];   //rescaled from extern when axis_steps_per_unit are changed by gcode
volatile float previous_speed[4]; // Speed of previous path line segment
volatile  float previous_nominal_speed; // Nominal speed of previous path line segment
...
extern void App_SDGets_Data(void);							//every time read one line
...
void plan_buffer_line(const float x, const float y, const float z, const float e, float feed_rate, const uint8_t extruder)
{
	...
	if(target[E_AXIS] != position[E_AXIS])
	{
		...
		if(degHotend(0) < EXTRUDE_MINTEMP && !allow_cold_extrude)
		{
			position[E_AXIS] = target[E_AXIS]; //behave as if the move really took place, but ignore E part
			SERIAL_ECHO_START;
			USR_ErrLog(MSG_ERR_COLD_EXTRUDE_STOP);
		}
		...
		if(labs(target[E_AXIS] - position[E_AXIS]) > axis_steps_per_unit[E_AXIS]*EXTRUDE_MAXLENGTH)
		{
			position[E_AXIS] = target[E_AXIS]; //behave as if the move really took place, but ignore E part
			SERIAL_ECHO_START;
			USR_ErrLog(MSG_ERR_LONG_EXTRUDE_STOP);
		}
		#endif
	}
	#endif
	...
	//����ͷ�����ж�
	block->laser = LaserSwitch;
	//USR_UsrLog("LaserSwitch is %d in plan\r\n", LaserSwitch);
	if(PrinterMode == 1)
		feed_rate = feed_rate / 2;
	#endif
...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
	...
	/*
	block->acceleration_st *x x���ݶ�ʱ����Ƶ�ʽ��м���;
	X:Խ��->
	X:ԽС->
	*/
	...
	#else
	block->acceleration_rate = (long)((float)block->acceleration_st * 6.194304);
	#endif
