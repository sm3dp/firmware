...
u8 minibmp_decode(u8 *filename, u16 x, u16 y, u16 width, u16 height, u16 acolor, u8 mode) //尺寸小于240*320的bmp图片解码.
{
	...
	datawrite = (u16 *)mymalloc(SRAMIN, readlen);
	if(datawrite == NULL)return PIC_MEM_ERR;		//内存申请失败.
					...
					for(j = 0; j < rowcnt; j++) //每次读到的行数
					{
						if(biCompression == BI_RGB) //RGB:5,5,5
						{
							for(i = 0; i < rowpix; i++)
							{
								...
								//tx++;
								datawrite[i] = ((u16) * bmpbuf & 0X1F);			//R
								datawrite[i] += (((u16) * bmpbuf++) & 0XE0) << 1; 	//G
								datawrite[i] += ((u16) * bmpbuf++) << 9;  	 //R,G
								tx++;
							}
							LCD_Color_Fill(x, y + ty, x + rowpix - 1, y + ty, datawrite);
						}
						else   //RGB 565
						{
							for(i = 0; i < rowpix; i++)
