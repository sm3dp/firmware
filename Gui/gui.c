...
extern const unsigned char asc2_1608[109][16];
...
u8 convert_tab[14] = {160, 164, 188, 156, 180, 169, 137, 168, 179, 177, 161, 163, 167, 173};
u8 convert_offset(u8 chr)
{
	u8 i = 0;
	for(i = 0; i < 13; i++)
	{
		if(convert_tab[i] == chr)
			break;
	}
	return i += 95;
}
...
void gui_show_ptchar(u16 x, u16 y, u16 xend, u16 yend, u8 offset, u16 color, u16 size, u8 chr, u8 mode)
{
	...
	if(chr > 94)//大于标准ascii为特殊符号
	{
		chr += ' ';
		chr = convert_offset(chr);
	}
...
void gui_show_ptstr(u16 x, u16 y, u16 xend, u16 yend, u16 offset, u16 color, u8 size, u8 *str, u8 mode)
{
	...
	while(*str != '\0')	//数据未结束
	{
		...
		{
			bHz = 0; //有汉字库
			if(*str == 0xc3 && (text_display.language_choice > 1))	//特殊西欧符号
			{
				if(EnDisp == 0)	//还未使能显示
				{
					if(offset >= size / 2)	//超过了一个字符
				...
				if(EnDisp == 1)	//使能显示
				{
					gui_show_ptchar(x, y, xend, yend, offset, color, size, *(str + 1), mode); //显示一个字符
					if((xend - x) > size / 2)x += size / 2;	//字符,为全字的一半
					else x += xend - x;					//未完全显示
					if(offset)
					{
...
void gui_show_string(u8 *str, u16 x, u16 y, u16 width, u16 height, u8 font, u16 fcolor)
{
	...
	while(*str != '\0') //未结束
	{
		...
		{
			if(*str == 0xc3)	//西欧特殊字符显示
			{
				if((xpos + 8) > (endx + 1)) //已经不在本行之内
				{
					xpos = x;
					ypos += font;
				}
				gui_show_ptchar(xpos, ypos, endx, endy, 0, fcolor, font, *(str + 1), 1);
				xpos += font / 2; //偏移
				str += 2;
			}
			else
